/*************************************************************************
Implementation File	: Building.h
Author Date			: V. Mavrommatis 24/11/2018
Purpose				: Header file of Building.cpp containing definitions
...
{
	private:
		int Ng_cap;//max ground floor capacity
		int cur_cap;//current ground floor capacity
		int level;//0 for ground
		int priority;//starts with 1
		int *waiting;//waiting list of citizens to board the lift
		Entry_Area *G_Entry;//entrance area where priorities are given
...
{
	private:
		int Nf_cap;//max floor capacity
		int No_cap;//max office capacity
		int cur_cap;//current floor capacity
	...
	public:
		Floor(int, int, int, int);//constructor
		~Floor();//destructor
...
class Elevator
{
	private:
		...
		int cur_cap;//lift current capacity
