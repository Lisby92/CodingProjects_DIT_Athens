...
main:   li $s0, 1000 # $s0 = size
        ...
        li $t1, 0    # $t1 = j == inner loop iterator and arrayS index
...
outer:  beq $t0, $s0, exit        # while i < size , no -> exit
          lb $t5, arrayS($t0)     # key = arrayS[i]
          add $t1, $t0, $s2       # j = 1 - 1
          lb $t3, arrayS($t1)     # load arrayS[j] element
inner1:   blt $t1, $zero, next    # while j >= 0, no -> next           
inner2:   ble $t3, $t5, next      # AND arrayS[j] > key, no -> next
            add $t4, $t1, $s1     # calculate j + 1 
            move $t6, $t3         # arrayS[j+1] = arrayS[j]
            sb $t6, arrayS($t4)   # store it at arrayS[j+1] position
            add $t1, $t1, $s2     # j = j - 1
            lb $t3, arrayS($t1)   # load arrayS[j] element
          j inner1                # -> inner1 , repeat
          nop
next:     add $t6, $t1, $s1       # calculate j + 1 after exiting inner loop  
          sb $t5, arrayS($t6)     # store the key @ arrayS[j+1]
          add $t0, $t0, $s1       # i = i + 1
        j outer 				  # -> outer , repeat
        nop
