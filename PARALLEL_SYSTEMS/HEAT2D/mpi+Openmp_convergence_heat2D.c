                                  ...
                                  const int flag)
{
  ...
  char filePath[70] = "";
  if(flag == 0)
    sprintf(filePath, "%d_%d_hybrid_serial_Initial.dat", rows, cols);
  else
    sprintf(filePath, "%d_%d_hybrid_serial_Final.dat", rows, cols);
...
int main(int argc, char *argv[])
{
    ...
    for(int steps = 0; steps < STEPS; ++steps)//main loop
    {
      ...
      if(steps % reductionStep == 0)
      {
        ...
        if(convResult == 0)//convergence detected
        {
          ...
          printf("\nConvergence detected by OpenMp threads at step %d\n", steps);
        }
      }
      else
  ...
  else /* PARALLEL MPI CODE */
  {
    ...
    sprintf(initFilePath, "%d_%d_hybrid_parallel_Initial%d.dat", ROWS, COLS, randTag);
    if(rank == 0)
    ...
    int localFlag;//to check for local convergence
    ...
    sprintf(finalFilePath, "%d_%d_hybrid_parallel_Final%d.dat", ROWS, COLS, randTag);
