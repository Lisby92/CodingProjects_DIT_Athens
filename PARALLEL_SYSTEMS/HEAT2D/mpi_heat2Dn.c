...
#define NXPROB      640                 /* y dimension of problem grid */
#define NYPROB      512                 /* x dimension of problem grid */
...
int main (int argc, char *argv[])
{
  ...
  MPI_Comm_rank(MPI_COMM_WORLD,&taskid);
   ...
   if (taskid == MASTER) {
      ...
      prtdat(NXPROB, NYPROB, u, "initial_heat.dat");
      ...
      double sum = 0;
      times = malloc(sizeof(double) * numworkers);
      if(times == NULL)
      {
        printf("ERROR: Failure of 'times' array allocation.\n");
        printf("Quitting...\n");
        MPI_Abort(MPI_COMM_WORLD, rc);
      }
      ...
      for (i=1; i<=numworkers; i++)
      {
         ...
         MPI_Recv(&times[i-1], 1, MPI_DOUBLE, source, TIME, MPI_COMM_WORLD, 
                  &status);
         sum += times[i-1];
      }
      ...
      prtdat(NXPROB, NYPROB, &u[0][0][0], "final_heat.dat");
   ...
   if (taskid != MASTER) 
   {
      ...
      MPI_Send(&u[iz][offset][0], rows*NYPROB, MPI_FLOAT, MASTER, DONE, 
               MPI_COMM_WORLD);
      double total_time = 1000 * (timeEnd - timeStart);
      MPI_Send(&total_time, 1, MPI_DOUBLE, MASTER, TIME, MPI_COMM_WORLD);
      MPI_Finalize();
   }
}
