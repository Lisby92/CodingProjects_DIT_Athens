...
__global__ void heatUpdateSerial(float * __restrict__ devHeatGrid)
{
  //declare dynamic shared memory
  HIP_DYNAMIC_SHARED(float, sharedHeatGrid)
...
__global__ void heatUpdateParallel(float       * __restrict__ devHeatGrid,
...
float * heatGrid = nullptr;
float * devHeatGrid = nullptr;
...
*/
static inline void heatInit(float * heatGrid)
{
  for(int i = 0; i < ROWS; ++i)//avoid halo area
    for(int j = 0; j < COLS; ++j)
      heatGrid[i*COLS+j] = (float) (i * (ROWS - i - 1) * j * (COLS - j - 1));
}

/* This function writes out the input grid to a .dat file in current path */
static inline int heatWrite(const float * heatGrid, const int flag, 
...
int main(int argc, char *argv[])
{
  ...
  const int warpOptSize = devProp.warpSize;
  const int sharedMemPerBlock = devProp.sharedMemPerBlock;

  //allocate host(CPU) memory
  heatGrid = (float *) malloc(GRID_SIZE * sizeof(float));
  if(heatGrid == nullptr)
  {
  ...
  HIP_CHECK(hipMalloc((void **)&devHeatGrid, GRID_SIZE * sizeof(float)));

  //transfer data from host(CPU) to device(GPU) memory
  HIP_CHECK(hipMemcpy(devHeatGrid, heatGrid, GRID_SIZE * sizeof(float),
  ...
  //calculate the memory needs of the problem
  int blockMemSize = GRID_SIZE * sizeof(float) * 2;
  int BLOCKS_PER_GRID = 1;
  int THREADS_PER_BLOCK = TOTAL_THREADS;
  while(blockMemSize <= sharedMemPerBlock)//find the number of blocks needed
  {
    BLOCKS_PER_GRID *= 2;
    blockMemSize /= 2;
    THREADS_PER_BLOCK /= 2;
  }
  if(BLOCKS_PER_GRID > THREADS_PER_BLOCK)
  {
    cout << "Number of requested threads < Number of threads per block\n";
    cout << "Aborting...\n";
    heatCleanExit(10);
  }
  else
    cout << "Kernel dimensions calculated\n";
  //calculate the kernel launch dimensions
  ...
  else//parallel execution
  {
    ...
    else if(cutF > (float) cut)//case 2.1 : handling 6, 8, 10, ... threads
    {
    ...
    //working and global area discovery phase
    //allocate device(GPU) global memory
  ...
  HIP_CHECK(hipMemcpy(heatGrid, devHeatGrid,
                      GRID_SIZE * sizeof(float), hipMemcpyDeviceToHost));
