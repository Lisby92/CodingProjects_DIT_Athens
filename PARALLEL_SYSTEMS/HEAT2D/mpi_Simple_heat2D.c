...
static inline void heatUpdateLocal(const double * old, double * new,
                                   const int cols,
...
static inline void heatUpdateSerial(const double * old, double * new,
                                    const int cols,
                                    const int rowStart, const int rowEnd, 
                                    const int colStart, const int colEnd)
{
                                  ...
                                  const int flag)
{
  ...
  char filePath[70] = "";
  if(flag == 0)
    sprintf(filePath, "%d_%d_simple_serial_Initial.dat", rows, cols);
  else
    sprintf(filePath, "%d_%d_simple_serial_Final.dat", rows, cols);
...
int main(int argc, char *argv[])
{
  ...
  else /* PARALLEL MPI CODE */
  {
    ...
    sprintf(initFilePath, "%d_%d_simple_parallel_Initial%d.dat", ROWS, COLS, randTag);
    if(rank == 0)
    ...
    /* CREATE THE COMMUNICATION DATATYPES */
    ...
    if(nn != MPI_PROC_NULL)
    {
      MPI_Recv_init(&(oldBlock[0*(blockCols+2)+1]), 1, row,
                    nn, S_TAG, cartComm, &(reqArrRcvOld[numReqRecvOld++]));
      MPI_Send_init(&(oldBlock[1*(blockCols+2)+1]), 1, row,
                    nn, N_TAG, cartComm, &(reqArrSendOld[numReqSendOld++]));
      MPI_Recv_init(&(newBlock[0*(blockCols+2)+1]), 1, row,
                    nn, S_TAG, cartComm, &(reqArrRcvNew[numReqRecvNew++]));
      MPI_Send_init(&(newBlock[1*(blockCols+2)+1]), 1, row,
                    nn, N_TAG, cartComm, &(reqArrSendNew[numReqSendNew++]));
    }
    if(ss != MPI_PROC_NULL)
    {
       MPI_Recv_init(&(oldBlock[(blockRows+1)*(blockCols+2)+1]), 1, row,
                     ss, N_TAG, cartComm, &(reqArrRcvOld[numReqRecvOld++]));
       MPI_Send_init(&(oldBlock[blockRows*(blockCols+2)+1]), 1, row,
                     ss, S_TAG, cartComm, &(reqArrSendOld[numReqSendOld++]));
       MPI_Recv_init(&(newBlock[(blockRows+1)*(blockCols+2)+1]), 1, row,
                     ss, N_TAG, cartComm, &(reqArrRcvNew[numReqRecvNew++]));
       MPI_Send_init(&(newBlock[blockRows*(blockCols+2)+1]), 1, row,
                     ss, S_TAG, cartComm, &(reqArrSendNew[numReqSendNew++]));
    }
    if(ww != MPI_PROC_NULL)
    {
      MPI_Recv_init(&(oldBlock[1*(blockCols+2)+0]), 1, col,
                    ww, E_TAG, cartComm, &(reqArrRcvOld[numReqRecvOld++]));
      MPI_Send_init(&(oldBlock[1*(blockCols+2)+1]), 1, col,
                    ww, W_TAG, cartComm, &(reqArrSendOld[numReqSendOld++]));
      MPI_Recv_init(&(newBlock[1*(blockCols+2)+0]), 1, col,
                    ww, E_TAG, cartComm, &(reqArrRcvNew[numReqRecvNew++]));
      MPI_Send_init(&(newBlock[1*(blockCols+2)+1]), 1, col,
                    ww, W_TAG, cartComm, &(reqArrSendNew[numReqSendNew++]));
    }
    if(ee != MPI_PROC_NULL)
    {
      MPI_Recv_init(&(oldBlock[1*(blockCols+2)+blockCols+1]), 1, col,
                    ee, W_TAG, cartComm, &(reqArrRcvOld[numReqRecvOld++]));
      MPI_Send_init(&(oldBlock[1*(blockCols+2)+blockCols]), 1, col,
                    ee, E_TAG, cartComm, &(reqArrSendOld[numReqSendOld++]));
      MPI_Recv_init(&(newBlock[1*(blockCols+2)+blockCols+1]), 1, col,
                    ee, W_TAG, cartComm, &(reqArrRcvNew[numReqRecvNew++]));
      MPI_Send_init(&(newBlock[1*(blockCols+2)+blockCols]), 1, col,
                    ee, E_TAG, cartComm, &(reqArrSendNew[numReqSendNew++]));
    }
    MPI_Barrier(cartComm);//all processes reach here for synchronization
    ...
    for(int steps = 0; steps < STEPS; ++steps)//main loop
    {
      ...
      /* DO THE LOCAL WORK */
      heatUpdateLocal(oldBlock, newBlock, blockCols,
    ...
    sprintf(finalFilePath, "%d_%d_simple_parallel_Final%d.dat", ROWS, COLS, randTag);
